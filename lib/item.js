...
function matchDescription(data) {
  let data =  data.match(/\"descriptions\"\:\[(.*?)\]/)[1].trim();
  data = Json.parse(data).value.replace(/<span style=\\\"color: #[A-Fa-f0-9]+\w\\\W>|<\\\/span>/, '');

  return data;
}

function matchParentItem(data) {
  return data.match(/skin for the (.*?) item/)[1].trim();
}
...
class Item {
  ...
  async getAllProperties() {
    ...
    return {
      ...
      parentItem: matchParentItem(data)
    };
  }
