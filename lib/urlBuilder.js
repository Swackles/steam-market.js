...
module.exports.generatePopular = (start, currency, count, appId, query = "") => {
  let url = `${steamMarket}search/render/?query&norender=1&currency=${getCurrency(currency)}&query=${encodeURIComponent(query)}&start=${start}&count=${count}&sort_column=popular`;
...
module.exports.generateSearch = (query, currency, appId) => {
  let url = `${steamMarket}search/render/?norender=1&currency=${getCurrency(currency)}&query=${encodeURIComponent(query)}`;
  if (appId) url += `&appid=${appId}`;
...
module.exports.generateInventory = (steamID64, appId, count, language) => {
  return `https://steamcommunity.com/inventory/${steamID64}/${appId}/2?l=${language}&count=${count}`;
}
