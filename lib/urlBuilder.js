...
function getCurrency(name) {
  let rawdata = fs.readFileSync('./config/currencies.json');
...
module.exports.generateHistogram = (itemId, currency) => {
  return `${steamMarket}itemordershistogram?norender=1&language=english&currency=${getCurrency(currency)}&item_nameid=${itemId}`
}
